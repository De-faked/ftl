...
export const StudentPortalPage: React.FC = () => {
  ...
  if (loading || studentLoading) {
    return <div className="p-6 text-sm text-gray-500">{t.portal.portalPage.loading}</div>;
  }
      ...
      <div className="max-w-6xl mx-auto">
        {studentError ? (
          <div className="rounded-2xl border border-red-100 bg-red-50 p-6 text-sm text-red-700 shadow-sm">
            <p className="font-semibold text-red-800">{t.portal.portalPage.loadErrorTitle}</p>
            <p className="mt-2"><Bdi>{studentError}</Bdi></p>
          </div>
        ) : student ? (
