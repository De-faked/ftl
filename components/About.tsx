...
export const About: React.FC = () => {
  ...
  return (
    <section className="py-12 sm:py-20 lg:py-24 bg-madinah-sand relative overflow-hidden" dir={dir}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div id="about" data-anchor="about" className="h-0"></div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            
          <div className="space-y-8">
            <h2 className="text-4xl font-serif font-bold text-madinah-green leading-tight rtl:font-kufi">
              {t.home.about.titleLine1} <br/>
              <span className="text-madinah-gold">{t.home.about.titleLine2}</span>
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="flex items-start gap-4 p-4 bg-white/60 rounded-xl backdrop-blur-sm">
                    ...
                    <div>
                        <h3 className="font-bold text-gray-900 rtl:font-kufi">{t.home.about.locationTitle}</h3>
                        <p className="text-sm text-gray-600 mt-1 rtl:font-amiri rtl:text-lg">{t.home.about.locationDesc}</p>
                    </div>
                </div>
                <div className="flex items-start gap-4 p-4 bg-white/60 rounded-xl backdrop-blur-sm">
                    <Users className="w-8 h-8 text-madinah-gold flex-shrink-0" />
                    <div>
                        <h3 className="font-bold text-gray-900 rtl:font-kufi">{t.home.about.teachersTitle}</h3>
                        <p className="text-sm text-gray-600 mt-1 rtl:font-amiri rtl:text-lg">{t.home.about.teachersDesc}</p>
                    </div>
                </div>
                <div className="flex items-start gap-4 p-4 bg-white/60 rounded-xl backdrop-blur-sm">
                    <GraduationCap className="w-8 h-8 text-madinah-gold flex-shrink-0" />
                    <div>
                        <h3 className="font-bold text-gray-900 rtl:font-kufi">{t.home.about.curriculumTitle}</h3>
                        <p className="text-sm text-gray-600 mt-1 rtl:font-amiri rtl:text-lg">{t.home.about.curriculumDesc}</p>
                    </div>
                </div>
                <div className="flex items-start gap-4 p-4 bg-white/60 rounded-xl backdrop-blur-sm">
                    <Heart className="w-8 h-8 text-madinah-gold flex-shrink-0" />
                    <div>
                        <h3 className="font-bold text-gray-900 rtl:font-kufi">{t.home.about.communityTitle}</h3>
                        <p className="text-sm text-gray-600 mt-1 rtl:font-amiri rtl:text-lg">{t.home.about.communityDesc}</p>
                    </div>
                </div>
            </div>
          </div>
...
/>
            ...
            <div className="absolute -bottom-6 -left-6 rtl:left-auto rtl:-right-6 bg-white p-6 rounded-lg shadow-xl max-w-xs">
                <p className="font-serif text-xl italic text-madinah-green mb-2 rtl:font-aref rtl:not-italic rtl:text-2xl">{t.home.about.imageCaption}</p>
